...
// Adicionando durações fictícias para enriquecer o visual da sidebar
...
const getCourseCategory = (name: string) => {
  ...
  if (n.includes('trauma') || n.includes('primeiros') || n.includes('atendimento')) return { label: 'Urgência', color: '#FBBF24', text: 'text-nexus-orange', border: 'border-nexus-orange/30' };
...
const PremiumView: React.FC<PremiumViewProps> = ({ userStats, onAddActivity, onAwardPoints, onIncrementUsage }) => {
  ...
  // Fix: Define filteredCourses to filter the current platform's courses by search
  ...
  const filteredLessons = useMemo(() => {
    ...
    // Fix: Filter lessons by search term
      ...
      <div className="fixed inset-0 z-[60] bg-nexus-bg flex flex-col md:flex-row animate-in fade-in duration-300">
        ...
        <div className="flex-grow flex flex-col overflow-y-auto no-scrollbar">
          ...
          <header className="h-16 shrink-0 bg-nexus-surface border-b border-nexus-border flex items-center justify-between px-6">
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                <p className="text-[10px] font-bold text-nexus-blue uppercase tracking-widest leading-none mb-1">{selectedCourse}</p>
          ...
          <div className="bg-black relative aspect-video w-full flex items-center justify-center group">
            ...
            {/* Custom Control Shadow Bar (Simulada para estética) */}
          ...
          <div className="p-6 md:p-10 max-w-5xl">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
              <div>
                ...
                <div className="flex items-center gap-4 text-sm font-medium text-nexus-text-sec">
                  <span className="flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> 12 min</span>
                  <span className="w-1 h-1 rounded-full bg-nexus-border"></span>
                  <span className="text-nexus-blue">{selectedPlatform} Academy</span>
                </div>
              </div>
            ...
            <div className="bg-nexus-surface border border-nexus-border rounded-3xl p-8">
               ...
               <div className="flex gap-4">
                  <button className="flex items-center gap-2 text-[10px] font-bold text-nexus-text-sec hover:text-nexus-blue uppercase tracking-widest transition-colors">
          ...
          <div className="p-6 border-b border-nexus-border bg-nexus-surface/50 backdrop-blur-sm sticky top-0 z-20">
            ...
            <div className="mt-4">
              <div className="flex justify-between items-center text-[9px] font-bold text-nexus-text-label uppercase tracking-widest mb-1.5">
                ...
                <span>{Math.round((watchedVideos.length / filteredLessons.length) * 100)}%</span>
              </div>
              <div className="h-1.5 w-full bg-nexus-bg rounded-full overflow-hidden border border-nexus-border/50">
                <div 
                  className="h-full bg-nexus-blue transition-all duration-1000" 
                  style={{ width: `${(watchedVideos.length / filteredLessons.length) * 100}%` }}
                ></div>
              </div>
            </div>
          </div>
                  ...
                  >
                    <div className="shrink-0 mt-0.5">
                      {isWatched ? (
                        ...
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center text-[10px] font-bold ${
                          isActive ? 'border-nexus-blue text-nexus-blue' : 'border-nexus-border text-nexus-text-label'
                        }`}>
                          {idx + 1}
                        </div>
                      )}
                    ...
                    <div className="min-w-0 flex-grow">
                      <p className={`text-xs font-bold leading-snug mb-1 ${
                        isActive ? 'text-nexus-blue' : 'text-nexus-text-main group-hover:text-nexus-text-title'
                      }`}>
                        {lesson.title}
                      </p>
                      <div className="flex items-center gap-2 text-[9px] font-bold text-nexus-text-label uppercase tracking-widest">
                        <span>Videoaula</span>
                        <span className="w-1 h-1 rounded-full bg-nexus-border"></span>
                        <span>{(lesson as any).duration || '15:00'}</span>
                      </div>
                    </div>

                    {isActive && (
                      <div className="w-1.5 h-1.5 rounded-full bg-nexus-blue animate-pulse mt-2 shadow-glow-blue"></div>
                    )}
                  </button>
                );
              })}
          ...
          {filteredLessons.map((lesson, idx) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <h3 className="text-sm font-semibold text-neutral-900 dark:text-nexus-text-title mb-2 tracking-tight leading-snug line-clamp-2 group-hover:text-sky-600 dark:group-hover:text-nexus-blue transition-colors">{lesson.title}</h3>
                  <div className="flex items-center text-[11px] font-medium text-neutral-400 dark:text-nexus-text-sec group-hover:text-sky-600 dark:group-hover:text-nexus-blue transition-colors">
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  <span className="text-[10px] font-medium text-nexus-text-sec mt-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">Ver Acervo <span className="transform group-hover:translate-x-1 transition-transform">→</span></span>
                </div>
              </div>
            );
          })}
            ...
            <div className="flex gap-4 md:gap-6 overflow-x-auto no-scrollbar px-4 pb-4">
              {cat.items.map((platform) => (
                <div 
                  ...
                  className="group relative flex-none w-[260px] md:w-[320px] aspect-video rounded-2xl bg-nexus-card border border-neutral-200/50 dark:border-nexus-border/50 overflow-hidden cursor-pointer transition-all duration-300 hover:scale-[1.03] hover:shadow-xl hover:z-10"
                >
                  <img 
                    src={platform.image} 
                    alt={platform.title} 
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent flex flex-col justify-end p-5 md:p-6 translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                    <span className="text-[9px] font-black text-nexus-blue uppercase tracking-[0.2em] mb-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300">Acesse Agora</span>
                    ...
                    <div className="absolute top-4 left-4">
                       <span className="bg-nexus-blue/20 backdrop-blur-md border border-nexus-blue/30 text-nexus-blue text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest shadow-sm">
                         Parceiro
                       </span>
                    </div>
                  </div>
